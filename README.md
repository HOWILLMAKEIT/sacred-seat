...
> 当前版本：`v0.1.5`。应用处于早期阶段，所有数据默认只保存在本机。
...
下面以“晚饭后不摄入热量”为例：下层是最终目标，上层是可以率先建立的具体行为。

![RSIP 国策树示例：将长期目标拆成可执行的上层节点](docs/images/policy-tree-example.png)
...
`v0.1.1` 是首个支持自动更新的版本；`v0.1.3` 起会自动读取 macOS 系统代理，并在启动、重新激活窗口或手动点击时检查更新。首次安装仍需手动下载。
